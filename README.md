...
- **AI-Powered Coaching** - Personalized tips and conversational coach using GPT-4o mini
...
- **OpenAI API** - GPT-4o mini for AI coaching and tips
...
│   ├── TipGenerator.swift        # AI tip generation
│   └── GoalCoach.swift           # Conversational AI coach
├── ViewModels/
│   └── DashboardViewModel.swift  # Dashboard business logic
├── Views/
│   ├── DashboardView.swift       # Main app screen with AI tips
...
- 🔒 Advanced statistics
- ✅ **AI Daily Tips** - Personalized saving tips generated by GPT-4o mini
- ✅ **AI Coach** - Chat with your AI savings coach for advice and motivation
...
### 3. Configure AI Features (OpenAI API)

The AI features (Daily Tips & Coach) require an OpenAI API key:
...
3. **API Key Security**
   ...
   - The key is stored in UserDefaults on device (local only)
   - For production: consider using a secure backend proxy
   - Current implementation: Direct API calls from device

4. **Cost Estimate**
   Using GPT-4o mini ($0.15/1M input, $0.60/1M output tokens):
   - Daily tip: ~$0.001 per generation
   - Coach conversation: ~$0.01-0.03 per session
   - Monthly cost for 1 user: ~$0.50-1.00
   - Premium subscription ($1.99/mo) easily covers costs

**For Development/Testing:**
Use the provided API key or add your own in Settings.
...
- **AI Daily Tip card** (Premium) - Personalized saving advice
- **AI Coach button** - Quick access to conversational coach
...
- Ask questions about saving strategies
